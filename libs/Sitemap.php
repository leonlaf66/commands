 ...
 */
class Sitemap extends \samdark\sitemap\Sitemap
{
    const ALWAYS = 'always';
    const HOURLY = 'hourly';
    const DAILY = 'daily';
    ...
    public function addItem($location, $lastModified = null, $changeFrequency = null, $priority = null)
    {
        ...
        $this->validateLocation($location[0]);
        
        $this->writer->writeElement('loc', $location[0]);
        if (count($location) > 1) {
            $this->writer->startElement('xhtml:link');
            $this->writer->writeAttribute('rel', 'alternate');
            $this->writer->writeAttribute('hreflang', 'zh');
            $this->writer->writeAttribute('href', $location[1]);
            $this->writer->endElement();
        }
